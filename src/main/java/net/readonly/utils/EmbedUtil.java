...
import net.readonly.core.listener.operation.InteractiveOperations;
import net.readonly.core.listener.operation.ReactionOperations;
import net.readonly.core.listener.operation.base.Operation;
import net.readonly.core.modules.commands.base.Context;

public class EmbedUtil {

	private static Config config = BotData.config();
    ...
    public static Future<Void> selectInt(GuildMessageReceivedEvent event, int max,
        ...
        return InteractiveOperations.create(event.getChannel(), event.getAuthor().getIdLong(), 30, (e) -> {
			...
			// Replace prefix because people seem to think you have to add the prefix before
			...
			if (message.toLowerCase().startsWith(config.getPrefix())) {
    ...
    public static Future<Void> list(GuildMessageReceivedEvent event, int timeoutSeconds, boolean canEveryoneUse, int length,
                                    	IntPairFunction<EmbedBuilder> supplier, String... parts) {
        if (parts.length == 0) {
            return null;
        }
    ...
    public static void listText(GuildMessageReceivedEvent event, int timeoutSeconds, boolean canEveryoneUse, int length,
                                        IntPairFunction<EmbedBuilder> supplier, List<String> parts) {
    ...
    private static List<MessageEmbed> buildSplitEmbed(IntPairFunction<EmbedBuilder> supplier, long length, String... parts) {
